...
__global__ void identify_home(T *data, uint32_t *cycles, const long long n_dtypes) {
    ...
    const size_t n_outer = ((sizeof(dtype) * n_dtypes) + inner_size - 1) / inner_size;
    const size_t n_inner = inner_size / (sizeof(dtype) * blockDim.x); // num of accesses per tb in inner loop
