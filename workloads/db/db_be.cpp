...
static void build_synthetic_ann_dataset(
    int Q, int N_COLD,
    const uint64_t *d_priority_hot_chunk_ptrs,
    const uint64_t *d_be_hot_chunk_ptrs0,
    const uint64_t *d_be_hot_chunk_ptrs1,
    float *d_query_vecs, float *d_cold_entries, int *d_candidate_ids)
{
    ...
    vector<uint64_t> h_priority_hot_ptrs(N_HOT_CHUNKS);
    vector<uint64_t> h_be_hot_ptrs0(N_HOT_CHUNKS);
    vector<uint64_t> h_be_hot_ptrs1(N_HOT_CHUNKS);
    gpuErrchk(hipMemcpy(h_priority_hot_ptrs.data(), d_priority_hot_chunk_ptrs,
                        sizeof(uint64_t) * N_HOT_CHUNKS, hipMemcpyDeviceToHost));
    gpuErrchk(hipMemcpy(h_be_hot_ptrs0.data(), d_be_hot_chunk_ptrs0,
                        sizeof(uint64_t) * N_HOT_CHUNKS, hipMemcpyDeviceToHost));
    gpuErrchk(hipMemcpy(h_be_hot_ptrs1.data(), d_be_hot_chunk_ptrs1,
                        sizeof(uint64_t) * N_HOT_CHUNKS, hipMemcpyDeviceToHost));
    for (int i = 0; i < N_HOT_CHUNKS; i++) {
        const float *src = h_hot_entries.data() + (size_t)i * DB_ENTRY_DIM;
        gpuErrchk(hipMemcpy(reinterpret_cast<void *>(h_priority_hot_ptrs[i]),
                            src, sizeof(float) * DB_ENTRY_DIM, hipMemcpyHostToDevice));
        gpuErrchk(hipMemcpy(reinterpret_cast<void *>(h_be_hot_ptrs0[i]),
                            src, sizeof(float) * DB_ENTRY_DIM, hipMemcpyHostToDevice));
        gpuErrchk(hipMemcpy(reinterpret_cast<void *>(h_be_hot_ptrs1[i]),
                            src, sizeof(float) * DB_ENTRY_DIM, hipMemcpyHostToDevice));
    }
...
struct VecScanBeArgs {
    const uint64_t *__restrict__ priority_hot_chunk_ptrs;
    const uint64_t *__restrict__ be_hot_chunk_ptrs0;
    const uint64_t *__restrict__ be_hot_chunk_ptrs1;
    const float    *__restrict__ cold_entries;
    const float    *__restrict__ query_vecs;
    const int      *__restrict__ candidate_ids;
    float          *__restrict__ priority_results;
    int            *__restrict__ priority_result_ids;
    float          *__restrict__ be_results;
    int Q;
    int N_COLD;
    int n_candidates;
    int be_engine_count;
...
struct VecScanBeTargetFn {
    __device__ __forceinline__
    void scan_query(const VecScanBeArgs *__restrict__ a,
                    const uint64_t *__restrict__ hot_chunk_ptrs,
                    int q, int tid, int blockDimX,
                    float *__restrict__ results,
                    int *__restrict__ result_ids) const
    {
        ...
        for (int ci = 0; ci < a->n_candidates; ci++) {
            ...
            if (cand_id < N_HOT_CHUNKS) {
                target = reinterpret_cast<const float *>(hot_chunk_ptrs[cand_id]);
            } else {
        ...
        if (tid == 0) {
            results[q] = best_score;
            if (result_ids) result_ids[q] = best_id;
        }
    }

    __device__ __forceinline__
    void operator()(const VecScanBeArgs *__restrict__ a,
                    int bid, int tid, int gridDimX, int blockDimX,
                    int n_ppnt_tbs_in_xcd) const
    {
        int xcd_lane = bid % XCD_NUM;
        if (xcd_lane != DB_VECSCAN_ACTIVE_XCD && xcd_lane != DB_BE_ACTIVE_XCD)
            return;
        ...
        if (xcd_lane == DB_VECSCAN_ACTIVE_XCD) {
            ...
            for (int q = logical_bid; q < a->Q; q += logical_grid_sz) {
                ...
                scan_query(a, a->priority_hot_chunk_ptrs, q, tid, blockDimX,
                           a->priority_results, a->priority_result_ids);
                if (tid == 0 && a->query_latencies && a->query_latencies[q] == 0) {
        ...
        for (int e = 0; e < a->be_engine_count; e++) {
            const uint64_t *hot_ptrs = (e & 1) ? a->be_hot_chunk_ptrs1
                                               : a->be_hot_chunk_ptrs0;
            for (int q = logical_bid; q < a->Q; q += logical_grid_sz)
                scan_query(a, hot_ptrs, q, tid, blockDimX, a->be_results, nullptr);
        }
    }
};
...
int main(int argc, char **argv) {
    ...
    const int be_engine_count = (argc > 2) ? max(0, atoi(argv[2])) : 1;

    printf("[DB] Q=%d N_TOTAL=%d N_HOT=%d N_COLD=%d DB_ENTRY_DIM=%d "
           "PRIORITY_XCD=%d PRIORITY_HBM=%d BE_XCD=%d BE_ENGINES=%d BE_HBM0=%d BE_HBM1=%d\n",
           Q, N_TOTAL, N_HOT, N_COLD, DB_ENTRY_DIM,
           DB_VECSCAN_ACTIVE_XCD, DB_VECSCAN_ORIGINAL_HBM,
           DB_BE_ACTIVE_XCD, be_engine_count, DB_BE_HBM0, DB_BE_HBM1);
    ...
    for (int v : {DB_VECSCAN_ORIGINAL_HBM, DB_BE_HBM0, DB_BE_HBM1}) {
    ...
    uint64_t *d_priority_chunk_ptrs =
        make_db_chunk_ptrs(db_chunks_per_hbm[DB_VECSCAN_ORIGINAL_HBM]);
    uint64_t *d_be_chunk_ptrs0 =
        make_db_chunk_ptrs(db_chunks_per_hbm[DB_BE_HBM0]);
    uint64_t *d_be_chunk_ptrs1 =
        make_db_chunk_ptrs(db_chunks_per_hbm[DB_BE_HBM1]);
    ...
    auto run_priority_with_best_effort = [&]() {
        int active_queries = count_vecscan_queries_for_xcd(
            Q, physical_grid_size, DB_VECSCAN_ACTIVE_XCD, 0);
        printf("\n\nPRIORITY VECSCAN + BEST-EFFORT LOAD\n");
        printf("[DB] priority: XCD%d HBM%d, active queries %d/%d\n",
               DB_VECSCAN_ACTIVE_XCD, DB_VECSCAN_ORIGINAL_HBM,
               active_queries, Q);
        printf("[DB] best-effort: %d engine(s), XCD%d, HBM%d/HBM%d alternating\n",
               be_engine_count, DB_BE_ACTIVE_XCD, DB_BE_HBM0, DB_BE_HBM1);
        printf("[DB] active priority VecScan queries on XCD%d: %d/%d\n",
        ...
        VecScanBeArgs h_args = {d_priority_chunk_ptrs, d_be_chunk_ptrs0, d_be_chunk_ptrs1,
                                d_cold_entries, d_query_vecs, d_candidate_ids,
                                d_results, d_result_ids, d_be_results,
                                Q, N_COLD, DB_CANDIDATES_PER_QUERY, 0,
                                d_blk_start, d_blk_end, d_blk_nq, d_q_lat, d_q_bid};
        VecScanBeArgs *d_args = nullptr;
        gpuErrchk(hipMalloc(&d_args, sizeof(VecScanBeArgs)));
        gpuErrchk(hipMemcpy(d_args, &h_args, sizeof(VecScanBeArgs), hipMemcpyHostToDevice));
        VecScanBeTargetFn fn{};
        size_t _n_plan = n_plan;
        void *kargs[] = {(void *)&fn, (void *)&d_args, (void *)&d_plan,
                         (void *)&_n_plan, (void *)&d_out};
        ...
        float solo_ms = launch_fused_and_time(
            (void *)ppnt::fused_kernel<VecScanBeTargetFn, VecScanBeArgs>,
        ...
        h_args.be_engine_count = be_engine_count;
        gpuErrchk(hipMemcpyAsync(d_args, &h_args, sizeof(VecScanBeArgs),
                                 hipMemcpyHostToDevice, stream));
        float corun_ms = launch_fused_and_time(
            (void *)ppnt::fused_kernel<VecScanBeTargetFn, VecScanBeArgs>,
            physical_grid_size, kargs, stream);
        char vecscan_tag[64];
        snprintf(vecscan_tag, sizeof(vecscan_tag), "PRIORITY XCD%d@HBM%d + BE%d XCD%d",
                 DB_VECSCAN_ACTIVE_XCD, DB_VECSCAN_ORIGINAL_HBM,
                 be_engine_count, DB_BE_ACTIVE_XCD);
