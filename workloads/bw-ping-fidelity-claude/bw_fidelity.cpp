    ...
    float* __restrict__ sink)
{
    ...
    // Accumulate all four components of all four loads so the result depends on the full 16B of
    // each load; combined with the inline asm below this stops the compiler from narrowing the
    // float4 loads to a single dword (fix #1: actually move 64B/thread/iter as accounted).
    float s0 = 0.f, s1 = 0.f, s2 = 0.f, s3 = 0.f;
    int cur_w = 0;
    int thr = is_target ? thr_cyc[0] : 0;             // steady per-iter delay (cycles)
    uint64_t cnt = 0, iter = 0;
    uint64_t win_end = window_cycles;                 // end of window cur_w, in cycles since t0

    // Fix #2: read the cycle counter and advance the window only once per STREAM_BATCH iterations
    // (no per-iteration s_memtime, no per-iteration 64-bit division). Window boundaries are tracked
    // by comparison against a running deadline instead of dividing. The target throttle stays
    // per-iteration because it is the intended load knob, not measurement overhead (probe: thr==0).
    for (;;) {
        const uint64_t elapsed = __builtin_readcyclecounter() - t0;
        if (elapsed >= win_end) {                     // crossed >= 1 window boundary
            if (tid == 0) outcnt[rlocal * W + cur_w] = cnt;
            cur_w++; win_end += window_cycles;
            while (elapsed >= win_end && cur_w < W) { cur_w++; win_end += window_cycles; }
            if (cur_w >= W) break;
            cnt = 0;
            thr = is_target ? thr_cyc[cur_w] : 0;
        }

        for (int kk = 0; kk < STREAM_BATCH; kk++) {
            // one iteration: each thread issues 4 independent 16B (dwordx4) loads = 64B total,
            // from distinct chunks. Inline asm forces full-width loads (see fix #1 above).
            const size_t base = (iter * (size_t)n_tbs_in_xcd + tbid_in_xcd) * groups + g;
            const size_t ci = base % nch;
            float4 r0, r1, r2, r3;
            asm volatile(
                "flat_load_dwordx4 %[O0], %[I0]\n\t"
                "flat_load_dwordx4 %[O1], %[I1]\n\t"
                "flat_load_dwordx4 %[O2], %[I2]\n\t"
                "flat_load_dwordx4 %[O3], %[I3]\n\t"
                "s_waitcnt vmcnt(0) & lgkmcnt(0)\n\t"
                : [O0]"=v"(r0), [O1]"=v"(r1), [O2]"=v"(r2), [O3]"=v"(r3)
                : [I0]"v"(&reinterpret_cast<float4*>(c0[ci])[off]),
                  [I1]"v"(&reinterpret_cast<float4*>(c1[ci])[off]),
                  [I2]"v"(&reinterpret_cast<float4*>(c2[ci])[off]),
                  [I3]"v"(&reinterpret_cast<float4*>(c3[ci])[off])
                : "memory");
