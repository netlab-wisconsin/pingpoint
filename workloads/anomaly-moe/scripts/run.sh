...
N_REQUESTS="${N_REQUESTS:-70}"
FIRST_START="${FIRST_ANOMALY_START:-14}"
FIRST_LENGTH="${FIRST_ANOMALY_LENGTH:-14}"
SECOND_START="${SECOND_ANOMALY_START:-42}"
SECOND_LENGTH="${SECOND_ANOMALY_LENGTH:-14}"

mkdir -p "$OUT/raw"
mkdir -p "$OUT/parsed"
STEM="anomaly_moe_n${N_REQUESTS}_a1_${FIRST_START}_${FIRST_LENGTH}_a2_${SECOND_START}_${SECOND_LENGTH}_first14_second_bpx10_full"
