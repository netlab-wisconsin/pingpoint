...
__global__ void k(
    ...
    size_t *k2_chunks_size, const size_t k2_chunk_size, const uint32_t k2_xcd,
    /* unused k2 args */
    uint32_t *cycles_start, uint32_t *cycles_stop)
{
    ...
    if (xcc_id == k1_xcd) { /* k1 */
        ...
        *k1_startClk = __builtin_readcyclecounter();
        ...
        // // set dst as tornado
        // const uint32_t XTO = 2; // hardcoded for this
        // const uint32_t xcd_dst = (xcc_id + XTO) % XCD_NUM;

        // // measurement
        // size_t n_iter = k2_chunks_size[xcd_dst] / n_tbs_in_xcd;
        // if (tid == 0) {
        //     #if DEBUG_LEVEL >= 1
        ...
        //     float4 *ptr_in1 = reinterpret_cast<float4*>(k2_chunks1[k2_offset1[xcd_dst] + chunk_idx]);
        //     float4 *ptr_in2 = reinterpret_cast<float4*>(k2_chunks2[k2_offset2[xcd_dst] + chunk_idx]);
        //     float4 *ptr_in3 = reinterpret_cast<float4*>(k2_chunks3[k2_offset3[xcd_dst] + chunk_idx]);
        //     float4 *ptr_in4 = reinterpret_cast<float4*>(k2_chunks4[k2_offset4[xcd_dst] + chunk_idx]);
