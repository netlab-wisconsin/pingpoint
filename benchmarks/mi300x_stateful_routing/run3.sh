  ...
  for k1_hbm in $(seq 0 2 6); do
    for k2_xcd in $(seq 1 2 7); do
      for k2_hbm in $(seq 1 2 7); do
        if [[ $k1_xcd -eq 4 && $k1_hbm -eq 0 ]]; then
          continue
        fi

        if [[ $k1_xcd -eq 4 && $k1_hbm -eq 2 && $k2_xcd -eq 1 ]]; then
          continue
        fi

        if [[ $k1_xcd -eq 4 && $k1_hbm -eq 2 && $k2_xcd -eq 3 && $k2_hbm -eq 1 ]]; then
          continue
        fi

        if [[ $k1_xcd -eq 4 && $k1_hbm -eq 2 && $k2_xcd -eq 3 && $k2_hbm -eq 3 ]]; then
          continue
        fi
