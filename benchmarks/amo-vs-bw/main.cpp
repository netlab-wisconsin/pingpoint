...
int main() {
    ...
    // for (int bpx = 160; bpx <= BPX_MAX; bpx += BPX_STRIDE) {
        ...
        for (int epoch = 0; epoch < NB_EPOCH; ++epoch) {
            ...
            double current_p00 = (double)h_latencySamples[0];
            double current_p25 = (double)h_latencySamples[totalPixels * 0.25];
            double current_p50 = (double)h_latencySamples[totalPixels * 0.50];
            double current_p75 = (double)h_latencySamples[totalPixels * 0.75];
            double current_p100 = (double)h_latencySamples[totalPixels - 1];

            // Incremental Average Update
            avg_p00 += (current_p00 - avg_p00) / (epoch + 1);
            avg_p25 += (current_p25 - avg_p25) / (epoch + 1);
            avg_p50 += (current_p50 - avg_p50) / (epoch + 1);
            avg_p75 += (current_p75 - avg_p75) / (epoch + 1);
            avg_p100 += (current_p100 - avg_p100) / (epoch + 1);
        }
