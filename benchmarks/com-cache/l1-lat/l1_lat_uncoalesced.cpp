// single warp pchase
// each thread access strided array elements to disable coalescing

// A single warp (64 threads) pchase, with each thread's memory access strided (to disable coalescing)
...
__global__ void l1_lat(uint32_t *startClk, uint32_t *stopClk, uint64_t *posArray, uint32_t posArraySize, uint32_t stride){
    ...
    uint64_t *ptr = posArray + (tid*stride % posArraySize);
    uint64_t ptr1, ptr0;
...
int main(){
    printf("single warp uncoalesced pchase: array_size=%d, repeat=%d, stride=%d\n", ARRAY_SIZE, REPEAT_TIMES, STRIDE);
