                    ...
                    uint32_t **cycles_stop) 
{
    ...
    if (tbid_in_xcd == 0 && tid == 0) {
        for (size_t i = 0; i < xcd_chunks_size[xcc_id]; i++) {
            ...
            float4 *ptr_in1 = reinterpret_cast<float4*>(xcd_chunks1[xcc_id][i]);
            float4 *ptr_in2 = reinterpret_cast<float4*>(xcd_chunks2[xcc_id][i]);
            float4 *ptr_in3 = reinterpret_cast<float4*>(xcd_chunks3[xcc_id][i]);
            float4 *ptr_in4 = reinterpret_cast<float4*>(xcd_chunks4[xcc_id][i]);
    ...
    for (size_t iter = 0; iter < n_iter; iter++) {
        ...
        float4 *ptr_in1 = reinterpret_cast<float4*>(xcd_chunks1[xcc_id][chunk_idx]);
        float4 *ptr_in2 = reinterpret_cast<float4*>(xcd_chunks2[xcc_id][chunk_idx]);
        float4 *ptr_in3 = reinterpret_cast<float4*>(xcd_chunks3[xcc_id][chunk_idx]);
        float4 *ptr_in4 = reinterpret_cast<float4*>(xcd_chunks4[xcc_id][chunk_idx]);
...
int main() {
    ...
    // const long long n_pages = 128; // 128MB per input data
    ...
    vector<uint32_t**> d_cycles(n_datas); // per-data, per-xcd, per-chunk. record cycles for home identification
    for (int i = 0; i < n_datas; i++) {
        gpuErrchk(hipMalloc((void**)&d_cycles[i], sizeof(uint32_t*) * XCDS_NUM ));
        for (int x = 0; x < XCDS_NUM; x++) {
            gpuErrchk(hipMalloc((void**)&d_cycles[i][x], sizeof(uint32_t) * n_chunks ));
        }
        printf("allocated d_cycles[%d] array[%d][%d]\n", i, XCDS_NUM, n_chunks);
    }

    vector<int*> d_home(n_datas); // per-data, per-chunk. record home xcd
    for (int i = 0; i < n_datas; i++) {
        gpuErrchk(hipMalloc((void**)&d_home[i], sizeof(int) * n_chunks ));
        printf("allocated d_home[%d] array[%d]\n", i, n_chunks);
    }
    ...
    for (int i = 0; i < n_datas; i++) {
        ...
        for (int x = 0; x < XCDS_NUM; x++) {
            gpuErrchk(hipMemcpy(h_cycles[x].data(), d_cycles[i][x], sizeof(uint32_t) * n_chunks, hipMemcpyDeviceToHost));
        }
    ...
    for (int i = 0; i < n_datas; i++) {
        gpuErrchk(hipMalloc((void**)&d_xcd_chunks[i], sizeof(uint64_t*) * XCDS_NUM));
        for (int x = 0; x < XCDS_NUM; x++) {
            size_t n_chunks = xcd_chunks[i][x].size();
            gpuErrchk(hipMalloc((void**)&d_xcd_chunks[i][x], sizeof(uint64_t) * n_chunks ));
            gpuErrchk(hipMemcpy(d_xcd_chunks[i][x], xcd_chunks[i][x].data(), sizeof(uint64_t) * n_chunks, hipMemcpyHostToDevice));
        }   
    }
    ...
    for (int i = 0; i < n_datas; i++) {
        for (int x = 0; x < XCDS_NUM; x++) {
            ...
            xcd_chunks_size[x] = min(xcd_chunks_size[x], xcd_chunks[i][x].size());
        }   
    }
