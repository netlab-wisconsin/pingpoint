...
int main() {
    ...
    for (int i = 0; i < n_datas; i++) {
        ...
        for (size_t k = 0; k < n_chunks; k++) {
            ...
            // Include each chunk to all to trigger uniform
            for (int x = 0; x < XCDS_NUM; x++) {
                h_xcd_chunks_size[i][x]++;
            }
        }
        #if DEBUG
        for (int x = 0; x < XCDS_NUM; x++) {
    ...
    for (int i = 0; i < n_datas; i++) {
        for (size_t k = 0; k < n_chunks; k++) {
            ...
            // Include each chunk to all to trigger uniform
            for (int x = 0; x < XCDS_NUM; x++) {
                xcd_chunks[i][x].push_back(reinterpret_cast<uint64_t>(d_data[i]) + k * (CHUNK_SIZE));
            }
        }
    }
    // safety check
    ...
    for (int i = 0; i < n_datas; i++) {
        ...
        for (int x = 0; x < XCDS_NUM; x++) {
            ...
            // printf("data[%d] xcd %d: offset %zu, n_chunks %zu\n", i, x, _offset, _n_chunks);
