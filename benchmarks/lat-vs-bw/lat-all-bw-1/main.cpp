...
#define K2_PINNED_HBM 1   // Kernel 2 pinned hbm
#endif
...
int main(int argc, char **argv) {
    ...
    for (int x = 0; x < XCD_NUM; x++) {
        string level = xcd_dtypes[x].size() * sizeof(dtype) > L2_SIZE ? 
                       (xcd_dtypes[x].size() * sizeof(dtype) > LLC_SIZE ? "hbm" : "llc") 
                       : "l2";
        assert (level != "l2"); // K1 data should be at least in LLC
        cout << "K1 pinned data: " << "hbm" << x << " "
             << xcd_dtypes[x].size() * sizeof(dtype) / (1024 * 1024) << "MB" << " "
             << "at " << level << " "
             << "\n" << flush;
    }
#endif
    ...
    for (int k2_bpx = K2_BPX_MIN; k2_bpx <= K2_BPX_MAX; k2_bpx++) {
...
#else
        const int64_t iters = (int64_t)100000; // for faster debugging
#endif
        ...
        const int64_t k2_iters = iters * 1;
        ...
        for (int epoch = 0; epoch < EPOCHS; epoch++) {
            ...
            GPU_ERROR(hipDeviceSynchronize());
#if DEBUG_LEVEL >= 0
            cout << "K1 warmup may introduce higher LLC hit rate" 
                 << "\n" << flush;
#endif
            ...
            uint64_t *d_k1_startClk, *d_k1_stopClk; 
            GPU_ERROR(hipMalloc((void**)&d_k1_startClk, sizeof(uint64_t) * EIGHT ));
            GPU_ERROR(hipMalloc((void**)&d_k1_stopClk, sizeof(uint64_t) * EIGHT ));

            // per-xcd, per-tb in xcd. record start/stop cycles for bw measurement
            uint64_t *d_k2_startClk, *d_k2_stopClk; 
            GPU_ERROR(hipMalloc((void**)&d_k2_startClk, sizeof(uint64_t) * k2_bpx ));
            GPU_ERROR(hipMalloc((void**)&d_k2_stopClk, sizeof(uint64_t) * k2_bpx ));
            ...
            k<dtype><<<k2_n_blocks, k2_n_threads_per_block>>>(
                ...
                k2_iters, d_k2_startClk, d_k2_stopClk
            );
            GPU_ERROR(hipDeviceSynchronize());
            ...
            vector<uint64_t> h_k2_cycles_start(k2_bpx);
            vector<uint64_t> h_k2_cycles_stop(k2_bpx);
            GPU_ERROR(hipMemcpy(h_k2_cycles_start.data(), d_k2_startClk, sizeof(uint64_t) * k2_bpx, hipMemcpyDeviceToHost));
            GPU_ERROR(hipMemcpy(h_k2_cycles_stop.data(), d_k2_stopClk, sizeof(uint64_t) * k2_bpx, hipMemcpyDeviceToHost));

            // for avg lat calc
            double avg_xcd_cycles = 0.0;
            // for global bw calc
            uint64_t min_xcd_cycles_start = UINT64_MAX;
            uint64_t max_xcd_cycles_stop = 0;
...
#if DEBUG_LEVEL >= 2
            for (int tbid_in_xcd = 0; tbid_in_xcd < k2_bpx; tbid_in_xcd++) {
                printf("epoch %d xcd %d tbid_in_xcd %d: k2 cycles %lu\n", epoch, K2_PINNED_XCD, tbid_in_xcd, 
                       (h_k2_cycles_stop[tbid_in_xcd] - h_k2_cycles_start[tbid_in_xcd]) );
            }
#endif
...
#if DEBUG_LEVEL >= 2
            cout << "epoch " << epoch << " xcd " << K2_PINNED_XCD << ": k2 bytes " << bytes << " "
                 << "global time sec " << global_time_sec << " "
                 << "global bw GB/s " << global_bw_GBps << " "
                 << "\n" << flush;
#endif
            ...
            vector<uint64_t> h_start(EIGHT);
            vector<uint64_t> h_stop(EIGHT);

            GPU_ERROR(hipMemcpy(h_start.data(), d_k1_startClk, sizeof(uint64_t) * EIGHT, hipMemcpyDeviceToHost));
            GPU_ERROR(hipMemcpy(h_stop.data(), d_k1_stopClk, sizeof(uint64_t) * EIGHT, hipMemcpyDeviceToHost));
            ...
            for (int x = 0; x < EIGHT; x++) {
                cout << "epoch " << epoch << " xcd " << x << ": k1 cycles " << (h_stop[x] - h_start[x]) << " "
                     << "\n" << flush;
            }
#endif
