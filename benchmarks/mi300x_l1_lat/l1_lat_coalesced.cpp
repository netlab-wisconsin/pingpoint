...
// A single warp (64 threads) pchase, with each thread's memory access coalesced
// Each array element points to +64 index array element
// Each thread starts pchase from array elements adjacent to each other (i.e., coalesced)
...
int main(){
    printf("single warp coalesced pchase: array_size=%d, repeat=%d\n", ARRAY_SIZE, REPEAT_TIMES);
