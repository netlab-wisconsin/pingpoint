...
// A single warp (64 threads) pcahse, with each thread accessing the same array element
// Each array element points to +1 index array element
// Each thread starts and continues pchase by accessing the same element
...
__global__ void l1_lat(uint32_t *startClk, uint32_t *stopClk, uint64_t *posArray, uint32_t posArraySize){
    ...
    // each thread starts from the same element
    // this is continued throughout the whole iterations
...
int main(){
    printf("single warp focused pchase: array_size=%d, repeat=%d\n", ARRAY_SIZE, REPEAT_TIMES);
