...
// Within a warp, each thread's memory access coalesced
...
__global__ void l1_lat(uint32_t *startClk, uint32_t *stopClk, uint64_t *posArray, uint32_t posArraySize)
{
    ...
    uint64_t *ptr = posArray + (wid * THREADS_PER_WARP) + wtid;
    uint64_t ptr1, ptr0;
...
int main(){
    printf("multi warp coalesced pchase: n_warps=%d, array_size=%d, repeat=%d\n", WARPS_PER_BLOCK, ARRAY_SIZE, REPEAT_TIMES);
