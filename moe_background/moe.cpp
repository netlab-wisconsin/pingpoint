...
int main(int argc, char **argv) {
...
#if 0 // disable add interferer 
        const int target_physical_blocks = max_blocks_per_sm * num_sms;
#else
        const int target_physical_blocks = max_blocks_per_sm * num_sms / 2;
#endif
    ...
    hipStream_t bg_stream;
    {
        ...
        gpuErrchk(hipDeviceGetStreamPriorityRange(&leastPrio, &greatestPrio));
        // On many platforms: "leastPrio" is lowest priority (largest number), "greatestPrio" highest.
        // Use low priority to reduce “starvation” risk (but still interferes).
        gpuErrchk(hipStreamCreateWithPriority(&bg_stream, hipStreamNonBlocking, leastPrio));
    }
    int* d_bg_stop = nullptr;
    gpuErrchk(hipMallocManaged(&d_bg_stop, sizeof(int)));
    *d_bg_stop = 0;
    gpuErrchk(hipDeviceSynchronize()); // ensure visibility before launch
...
#endif
    *d_bg_stop = 1;
    gpuErrchk(hipDeviceSynchronize());      // ensure flag is visible device-side reasonably quickly
